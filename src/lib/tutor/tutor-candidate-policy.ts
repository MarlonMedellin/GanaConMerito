...
export function evaluateTutorCandidatePolicy(params: {
  ...
  if ((execution.outputTokens ?? 0) > MAX_OUTPUT_TOKENS) return reject("output_token_limit");
  if ((execution.costUsd ?? 0) > MAX_TURN_COST_USD) return reject("turn_budget_exceeded");
  if (((params.budget?.sessionCostUsd ?? 0) + (execution.costUsd ?? 0)) > MAX_SESSION_COST_USD) return reject("session_budget_exceeded");
...
function isEvidenceKeyAvailable(key: string, evidence: TutorEvidence) {
  if (key === "question" || key === "source_evidence") return Boolean(evidence.question);
