...
export const MAX_PREPARATION_MAP_ROWS = 3;
...
export function getPreparationMapRows(rows: DashboardTopicBreakdownRow[]) {
  return [...getEligibleInsights(rows)]
    .sort((a, b) => {
      const attemptsDiff = b.row.attempts - a.row.attempts;
      if (attemptsDiff !== 0) return attemptsDiff;
      return a.accuracy - b.accuracy;
    })
    .slice(0, MAX_PREPARATION_MAP_ROWS);
}
