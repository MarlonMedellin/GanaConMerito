-- Reconciles production migration history with the governed V4 importer.
-- Keeps the historical overload intact and adds the current idempotent contract.
...
comment on function public.upsert_content_item_v4(jsonb, text, text, text) is
  'Imports an approved V4 item idempotently and keeps it inactive until a separate Canary activation gate.';
