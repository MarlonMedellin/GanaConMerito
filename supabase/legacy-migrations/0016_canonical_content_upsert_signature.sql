-- 0016_canonical_content_upsert_signature.sql
-- Leaves one unambiguous upsert signature for beta items.

begin;

drop function if exists public.upsert_content_item(
  text, text, text, text, text, text, text, numeric, text, text,
  text, text, text, text[], boolean, integer, jsonb, text
);
...
begin
  insert into public.item_bank (
    ...
    is_published, version, source_path, status, is_active, editorial_metadata
  ) values (
  ...
  on conflict (slug) do update set
    ...
    is_active = excluded.is_active, editorial_metadata = excluded.editorial_metadata,
    updated_at = now()
